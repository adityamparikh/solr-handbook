...
- **January 2021** — Elastic relicensed Elasticsearch and Kibana from Apache 2.0 to dual SSPL/Elastic License v2 to block AWS from offering a managed service. The Open Source Initiative does not consider either license open-source.
...
| **Tool surface (core)** | `search`, `index-json/csv/xml`, `list-collections`, `get-collection-stats`, `check-health`, `create-collection`, `get-schema` | `search`, `list_indices`, `get_index`, `get_mappings`, `esql` (the Elasticsearch Query Language) | `SearchIndexTool`, `IndexMappingTool`, `ListIndexTool`, `ClusterHealthTool`, plus model-tool for ML inference |
| **Distinctive feature** | OAuth2 across multiple IdPs; OTel/LGTM observability built in; clean Java integration | First-class **ES|QL** support — the LLM can write piped queries directly; deep tie-in with Elastic's AI Search features (semantic_text, ELSER) | First-class **neural search** tools that route through OpenSearch's built-in ML Commons; Bedrock and SageMaker integration out of the box |
