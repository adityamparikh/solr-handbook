...
For our `text_en` above, indexing the title `"Stranger Things Season 4"` produces `[stranger, thing, season, 4]` — the stage-by-stage trace is in the Analysis-form walkthrough just below, where you can reproduce it against a live Solr.
...
The official image is the Docker Official Image [`solr`](https://hub.docker.com/_/solr) on Docker Hub (there is no `apache/solr` repository). Tags follow Solr releases: `solr:10.0.0`, `solr:9.9`, `solr:latest`, plus `-slim` variants. The Solr Admin UI is available on port 8983 — useful for poking at a running container while debugging a test.
