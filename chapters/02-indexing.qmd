...
>
> **Companion code:** every pattern in this chapter — the seed catalog, the Schema API setup, bulk indexing, atomic/in-place updates, optimistic concurrency, the cursorMark reindexer, and the alias swap — is runnable and Testcontainers-tested in [`code/src/main/java/solrbook/indexing/`](https://github.com/adityamparikh/solr-handbook/tree/main/code).
...
|---|---|---|
| **Docker** | You want a repeatable, isolated runtime — the default recommendation. | `docker run -p 8983:8983 solr:10.0.0`, then `docker exec <container> solr create -c shows` |
| **Homebrew (macOS)** | You want Solr managed alongside your other dev tools. | `brew install solr && brew services start solr` — note the brew service starts Solr in *standalone* mode (`--user-managed`), so the SolrCloud examples in this book need `bin/solr start` instead |
| **Archive** | You want to read the on-disk layout, edit `solr.in.sh`, or run on Linux without Docker. | Download from [solr.apache.org/downloads](https://solr.apache.org/downloads.html), then `bin/solr start` (SolrCloud is the default in Solr 10) |
...
The Docker image defaults to `SOLR_HOME=/var/solr/data`; the tarball archive defaults to `server/solr` (the systemd service install uses `/var/solr/data`). Homebrew uses `/opt/homebrew/var/lib/solr` on Apple Silicon. Know where it is — when an integration test misbehaves, the on-disk index is the source of truth. (The Docker image's `solr-precreate` helper still exists, but it starts Solr in *standalone* mode — not what this book's SolrCloud examples assume.)
...
If that doesn't return `10.0.0`, check `solr-8983-console.log` in the logs directory (`SOLR_LOGS_DIR` — `server/logs` for the archive, `/var/solr/logs` in Docker). For multi-node tests, the [Testcontainers pattern in §2.7](#integration-testing-with-testcontainers) is usually simpler than running multiple `bin/solr` processes by hand.
...
> **Production tip — don't expose the admin UI to the public internet.** It's a development tool. In production, fence it off with a reverse proxy that requires authentication, or turn it off entirely by uncommenting `SOLR_UI_ENABLED=false` in `solr.in.sh` (SOLR-14014; the companion `SOLR_UI_EXPERIMENTAL_ENABLED` switch controls only the new experimental UI that Solr 10 ships alongside the classic one). Note also that Solr binds to the **loopback interface** (`127.0.0.1`) by default — `SOLR_HOST_BIND` / `-Dsolr.host.bind` is what opens it up, and you should widen it no further than your application subnet. The production observability stack lives in §4.14, and cluster-wide authentication in §4.13.
...
> **Production tip — shared configsets share schema edits.** If you create a collection *without* naming a configset, Solr copies `_default` into a fresh per-collection configset. If you explicitly bind several collections to the *same* configset (including `_default` itself), every Schema API change you make through one collection applies to all of them — and a second "create the same fields" setup script fails with "field already exists". Bind collections to a shared configset only when you *want* them schema-locked together; otherwise give each collection its own.
...
```bash
bin/solr create -c shows -n shows_config --shards 2 --replication-factor 2
```

Watch the flags — Solr 10's CLI renamed several of them and the short forms are not the ones muscle memory expects:

| Flag | Long form | Meaning |
|---|---|---|
| `-c` | `--name` | Collection (or core) name |
| `-n` | `--conf-name` | Name of an **existing** configset in ZooKeeper — this is what the command above uses |
| `-d` | `--conf-dir` | A **local directory** to upload as a new configset (or one of the bundled configsets, e.g. `_default`) |
| `-sh` | `--shards` | Number of shards |
| `-rf` | `--replication-factor` | Replicas per shard |
| `-s` | `--solr-url` | Base Solr URL — **not** shards |

: The `bin/solr create` flags in Solr 10 and their short forms. `-n` reuses a configset already in ZooKeeper; `-d` uploads one from disk. {#tbl-02-create-flags}

Mixing up `-n` and `-d` is the common trip-up: `-d shows_config` looks like "use the `shows_config` configset" but actually means "upload the local directory `shows_config`", and fails if no such directory exists under the Solr install.
...
<schema name="shows" version="1.7">
  ...
  <!-- hnswM / hnswEfConstruction were named hnswMaxConnections / hnswBeamWidth in Solr 9.x -->
  <fieldType name="knn_vector_384" class="solr.DenseVectorField"
             vectorDimension="384" similarityFunction="cosine"
             knnAlgorithm="hnsw" hnswM="16" hnswEfConstruction="100"/>
  ...
  <!-- copyField destination: stored=false AND useDocValuesAsStored=false, so atomic
       updates (§2.8.1) don't reconstruct it twice — once from the copy, once from
       docValues. StrField defaults useDocValuesAsStored to true; you must say no. -->
...
That's 30 shows across 8 platforms, ~20 distinct genres, English and non-English titles (`Shōgun`, `Squid Game`), running and ended status, and franchise relationships (`TM-001` and `AND-001` share the Star Wars universe as `franchise_id = "STAR-WARS"`, while `LK-001` is Marvel — `franchise_id = "MCU"`; both groupings are useful for the `relatedness()` aggregation in §5.5).
...
The Solr 10 reference guide states verbatim: *"If you want to use Jetty-based HTTP clients (HttpJettySolrClient, ConcurrentUpdateJettySolrClient, LBJettySolrClient, or CloudJettySolrClient), you need to add the maven dependency solr-solrj-jetty."* Alternatively, *"you can use HttpJdkSolrClient from the base solr-solrj artifact, which uses Java's built-in HTTP client and has no additional dependencies."* The Jetty split is the result of SOLR-17161, which landed in Solr 10.0; further artifacts include `solr-solrj-zookeeper` (only needed if you really must speak ZooKeeper from the client) and `solr-solrj-streaming` (Streaming Expressions, §5.2). Both were transitive dependencies of `solr-solrj` in 9.x and are opt-in in 10 — part of the long-running SolrJ modularization effort tracked under SOLR-15730.

> **Solr 9 difference — SolrJ packaging and package names.** In Solr 9.x the Jetty clients lived inside `solr-solrj` itself, and the separate `solr-solrj-zookeeper`/`-streaming` modules came in transitively. Solr 10 moved the Jetty clients into `solr-solrj-jetty` (SOLR-17161) and made the other modules opt-in from a Maven-POM perspective. Two renames come with it: `Http2SolrClient` → `HttpJettySolrClient` and `ConcurrentUpdateHttp2SolrClient` → `ConcurrentUpdateJettySolrClient`, both now in `org.apache.solr.client.solrj.jetty` rather than `…solrj.impl` (SOLR-18005). Separately, `SolrQuery` moved from `org.apache.solr.client.solrj` to `org.apache.solr.client.solrj.request`. If you're upgrading a 9.x build script, expect to add `solr-solrj-jetty` (or switch to `HttpJdkSolrClient`), possibly `solr-solrj-zookeeper`, and to fix those imports.

> **Production tip — Solr 10 SolrJ runs on Java 17.** Per the Solr 10 upgrade notes, the minimum Java version for the Solr 10 *server* is Java 21, while the minimum for *SolrJ* 10 is Java 17. Java 17 client applications can still talk to a Solr 10 cluster.

### Connecting to SolrCloud

In Solr 10, the recommended pattern is to connect via Solr node URLs, not directly to ZooKeeper. The ZooKeeper-hosts constructor on `CloudSolrClient.Builder` still compiles without a warning in 10.0.0 — its formal deprecation (SOLR-14070) is scheduled for **Solr 10.1** — but the guidance is already to give the client a list of Solr base URLs, and doing so also means you don't need the `solr-solrj-zookeeper` artifact on your classpath.
...
public class SolrFactory {
    public static SolrClient build() {
        ...
        // Timeouts live on the internal HTTP client's builder, not on
        // CloudSolrClient.Builder itself. Use withHttpClientBuilder — the older
        // withInternalClientBuilder is @Deprecated(since = "9.10") and just delegates.
        return new CloudSolrClient.Builder(solrUrls)
            .withDefaultCollection("shows")
            .withHttpClientBuilder(new HttpJdkSolrClient.Builder()
...
> **Solr 9 difference — ZooKeeper host constructor.** In Solr 9.x, `new CloudSolrClient.Builder(zkHosts, Optional.of(chroot))` was the common idiom: the client connected directly to ZooKeeper. It still works in Solr 10.0 and is not yet annotated `@Deprecated` — that lands in 10.1 (SOLR-14070), with removal unlikely before Solr 11 — but the recommendation is to pass Solr base URLs as shown above (the client discovers the cluster via the Solr nodes themselves). If you absolutely must speak ZK from the client, add `solr-solrj-zookeeper` and use the ZK-host constructor — but the URL-list form is the future and is what works in production without the extra dependency.
>
...
// The Jetty clients live in the org.apache.solr.client.solrj.jetty package
// (solr-solrj-jetty module), and the builder takes the HTTP client to send
// through; `true` closes that client when the concurrent client closes.
ConcurrentUpdateJettySolrClient.Builder builder =
    new ConcurrentUpdateJettySolrClient.Builder(
            ...
            true);
builder.withQueueSize(10_000).withThreadCount(8);

try (ConcurrentUpdateJettySolrClient client = builder.build()) {
...
> **Production tip — set `withQueueSize` deliberately.** Too small and threads stall on the bounded queue. Too large and a node failure leaves thousands of in-flight docs to recover. 5k–20k is a typical sweet spot. Monitor update-handler add rates via `/admin/metrics` (OpenTelemetry/Prometheus snake_case naming in Solr 10; the Dropwizard-era key was `UPDATE.updateHandler.adds`).
...
[Testcontainers](https://testcontainers.com) is a Java library that spins up real Docker containers inside your JUnit or TestNG tests and tears them down automatically when the test suite finishes. The `solr` module provides a `SolrContainer` that starts the official `solr` Docker image, exposes the Solr port, and lets you connect with the same `SolrClient` you use in production.

> **Why not an embedded Solr?** There is no supported embedded Solr in Solr 10. The old `EmbeddedSolrServer` still exists (Solr's own test framework uses it), but it is not documented in the reference guide, is not a supported deployment mode, and lacks SolrCloud semantics. Testing against a real Solr is the dependable path, and Testcontainers is how you do that without a shared dev cluster.

### The official Docker image

The official image is the Docker Official Image [`solr`](https://hub.docker.com/_/solr) on Docker Hub (there is no `apache/solr` image on Docker Hub; the Dockerfiles themselves live in the Apache-maintained [`apache/solr-docker`](https://github.com/apache/solr-docker) repo). Tags follow Solr releases: `solr:10.0.0`, `solr:9.9`, `solr:latest`, plus `-slim` variants. The Solr Admin UI is available on port 8983 — useful for poking at a running container while debugging a test.

> **Admin UI for debugging.** When a test is failing and you want to inspect the index, add a `Thread.sleep(300_000)` breakpoint, connect to `http://localhost:<mappedPort>/solr`, and use the **Analysis** tab (to trace how a string is tokenized), the **Query** tab (to run ad-hoc queries), and the **Schema** tab (to inspect field definitions). It's the fastest way to diagnose analyzer chain or schema issues without writing more test code. See the [Solr Admin UI reference](https://solr.apache.org/guide/solr/latest/getting-started/solr-admin-ui.html) for the full tour.
...
(Testcontainers 2.x renames the artifact to `testcontainers-solr` and moves the class to the `org.testcontainers.solr` package; this book pins the last 1.x line, which is what the Solr module currently ships on.)
...
class ProductIndexTest {
    ...
    static SolrContainer solr = new SolrContainer(
            DockerImageName.parse("solr:10.0.0"))
    ...
    void setUp() {
        ...
        client = new HttpJettySolrClient.Builder(url)
            .withDefaultCollection("shows")
            .build();
    }
...
- **Use a base-URL HTTP client, not `CloudSolrClient`.** The container runs SolrCloud (embedded ZooKeeper) and registers its node as `localhost:8983_solr` — the *container-internal* address. A `CloudSolrClient` would fetch cluster state and route requests to that unreachable address. An `HttpJettySolrClient` (or `HttpJdkSolrClient`) pointed at the mapped port works for everything, including the Collections and Schema APIs. In production — where nodes advertise reachable addresses — `CloudSolrClient` is the right default.
- **`withCollection("shows")`** — creates a collection using Solr's default configset. To use a custom configset, upload one with `.withConfiguration("shows_config", getClass().getResource("/solrconfig.xml"))` plus optional `.withSchema(...)` — see the [Testcontainers Solr docs](https://java.testcontainers.org/modules/solr/).
...
> **Production tip — pin the image version.** `solr:latest` will silently pull a newer Solr on the next `docker pull`, which can break tests when APIs change. Pin to `solr:10.0.0` (or whatever minor you're targeting) in CI, and update deliberately.
...
The Solr 10 reference guide states the rule exactly: *"all fields in your schema must be configured as stored (`stored="true"`) or docValues (`docValues="true"`) except for fields which are `<copyField/>` destinations, which must be configured as `stored="false"` and either `docValues="false"` or `useDocValuesAsStored="false"`."*

The reason for the `copyField` carve-out: on every atomic update Solr reconstructs the whole document from what it can read back, then reindexes it — which re-runs `copyField`. If the destination is *also* readable (stored, or docValues-backed with `useDocValuesAsStored="true"`), the reconstructed document already contains the destination's old values, and the copy adds them again. The symptom is a multi-valued field that grows by one duplicate on every update, or a single-valued one that rejects the update outright. This is why `title_str` and `cast_str` in §2.3 carry `useDocValuesAsStored="false"` — `StrField` with `docValues="true"` defaults it to `true`, which is exactly the shape that breaks.
...
> **Production tip — fail fast with `update.partial.requireInPlace=true`.** This request parameter (SOLR-15150, available since Solr 8.9/9.0) makes an atomic update that *can't* be done in-place fail outright instead of silently falling back. Use it on hot counter paths in your CI tests — the day someone "helpfully" adds `stored="true"` to your counter field, the build breaks instead of production performance.
...
public void bumpViewCount(SolrClient client, String docId) throws Exception {
    ...
    req.add(doc);
    req.setParam("update.partial.requireInPlace", "true"); // fail fast (SOLR-15150, 8.9+)
...
1. **The schema needs `_root_`** as an indexed field (Solr auto-populates it with the root document's ID). The usual atomic-update storage requirements (§2.8.1) apply to your real fields.
...
Caveats: an *atomic* update on a child document reindexes the entire tree from the root — expect that overhead; an *in-place* update (an eligible numeric docValues-only field, per §2.8.2) is exactly how you avoid it. And delete-by-id only works on root documents — to delete a child, use delete-by-query or an atomic remove on the parent's children pseudo-field.

### 2.8.5 Querying nested documents: `{!parent}` and `{!child}` (briefly)

If you do adopt the show-with-nested-episodes granularity from @tbl-02-doc-granularity, you also need the query side: the **block-join query parsers**. Nested documents are stored as a contiguous *block* in the index (children first, parent last — that's what `_root_` pins together), and the block-join parsers exploit that layout to cross levels in a single query, no application-side join.

Two parsers, mirror images of each other:

```text
# Parents from children: shows with at least one episode rated ≥ 9
q={!parent which="doc_type_s:show"}episode_rating_f:[9 TO *]

# Children from parents: the episodes of every ongoing show
q={!child of="doc_type_s:show"}status:ongoing
```

- `{!parent which=<allParents>}<childQuery>` returns **parent** documents whose children match the child query.
- `{!child of=<allParents>}<parentQuery>` returns the **child** documents of parents matching the parent query.

**The gotcha that corrupts results — `which`/`of` is a *filter*, not your search.** Both parameters must be a query that matches **every parent document in the index** (and no children) — a structural marker like `doc_type_s:show`, *not* the thing you're searching for. Putting the user's query in `which` is the classic block-join mistake; because the parser uses it to build a bitmask of parent positions, getting it wrong silently mismatches parents to other documents' children or throws cryptic `child query must only match non-parent docs` errors. Keep a dedicated discriminator field (`doc_type_s`) on every parent for exactly this purpose.

To return matched parents *with* their children in one response, add the `[child]` document transformer to `fl`:

```text
q={!parent which="doc_type_s:show"}episode_rating_f:[9 TO *]
&fl=id,title,[child limit=5 fl=id,episode_title_s]
```

That's the whole surface you need for the catalog use case. The deeper end — `childFilter` on the transformer, multi-level nesting addressed via `_nest_path_`, combining block-join with faceting (`uniqueBlock()`) — lives in the reference guide's [searching-nested-documents](https://solr.apache.org/guide/solr/latest/query-guide/searching-nested-documents.html) page. And remember the §2.2 warning before reaching for any of it: nested documents buy single-query convenience at the price of block-integrity maintenance — every tree update rewrites the whole block (§2.8.4), and a flat schema with an application-side join is often the better production trade.
...
Solr ships a `REINDEXCOLLECTION` command that does steps 1 and 2 from the server side:
...
It creates the target collection with the new schema, copies the source documents via their **stored fields**, and runs them through the new analyzer chain. The alias behavior depends on the target: only when the target name is the *same as the source* (or omitted) does Solr generate a sequentially-named target and alias the original name to it; with an explicit `.setTarget("shows_v2")` as above, **no alias is created** — the CREATEALIAS swap remains your step. The Javadoc carries an unambiguous warning: *"Reindexing is potentially a lossy operation — some indexed data that is not available as stored fields may be irretrievably lost."*
...
- **Test against a real Solr.** Use `SolrContainer` from Testcontainers with the official `solr:10.0.0` image and a base-URL HTTP client. One `static` container per test class; clean the index in `@BeforeEach` rather than restarting.
- Partial updates come in three flavors: **full replace** (default), **atomic** (server-side modifier maps — still a full reindex underneath), and **in-place** (genuine docValues-only update for single-valued numeric fields). Use `update.partial.requireInPlace=true` (SOLR-15150, Solr 8.9+) to fail fast when an in-place update would silently fall back to atomic.
