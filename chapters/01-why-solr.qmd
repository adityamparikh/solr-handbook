...
> **What this chapter covers and why it matters.** Before you adopt any search engine, you should be able to defend, in one paragraph, why a B-tree index over `VARCHAR` columns is *not* the right tool for the job. This chapter builds that intuition with a concrete TV-shows catalog example, walks through the inverted-index data structure as it actually lives in a Lucene segment, and shows step-by-step how each kind of query (term, phrase, prefix, wildcard, fuzzy, range, boolean) is executed against those structures. It ends with a decision framework: Solr vs. SQL vs. a vector database vs. PostgreSQL FTS.
>
...
Looking up `stranger` in the sorted term dictionary is cheap regardless of corpus size, and `stranger AND thing` is an intersection of two sorted doc-id lists driven by the smaller one — fast even at billions of documents. (§1.3 covers the structures that make this true.) Notice also that Porter stemming maps `strange` to `strang` but `strangers` only to `stranger` (the plural `-s` is stripped; `-er` never is) — three related surface forms, two distinct roots that never collapse together. This is a real quirk of the English Porter stemmer and a recurring source of "why didn't my synonym work?" confusion (covered in §2.4).
...
Now we can walk through the common query types and explain *which of the five structures they read* and *why their cost differs*.
...
2. Look the term up in the term dictionary, which points at its posting list.
3. Iterate the postings; score each live document with BM25 (using the per-doc length norms); skip tombstoned docs.
...
1. Look up both terms and get their posting lists *with* position data.
2. Run the AND intersection as before to find candidate docs.
3. For each candidate, decode the positions of both terms in that doc. Look for a position `p_stranger` and a position `p_things` such that `p_things == p_stranger + 1` (or within `slop` for `"stranger things"~2`).
4. Score only the docs that pass the position check.

**Cost:** AND intersection cost + position-stream decode per candidate. Phrase is meaningfully more expensive than the underlying AND. **Implication:** index positions only on fields where you need phrase/proximity; `omitPositions="true"` drops the position data entirely.
...
> **Production tip — the `maxBooleanClauses` trap.** Solr caps boolean clause counts (default 1024 — since Solr 9 enforced node-wide in `solr.xml` and per-collection in `solrconfig.xml`). Prefix and wildcard queries use a constant-score rewrite *by default*, so they rarely hit the cap; you'll see `TooManyClauses` mainly with scoring rewrites or machine-generated boolean queries. If you legitimately need huge boolean queries, raise the cap deliberately — and know why.
...
**Cost:** dominated by how many terms fall within the edit-distance window. Short query terms with `~2` produce huge candidate sets; longer terms produce small ones. Requiring the first character(s) to match exactly — `prefixLength=1` or `2` on the dedicated fuzzy query parser, e.g. `{!fuzzy f=title prefixLength=2}thinngs` — is the standard way to keep fuzzy queries tractable on large dictionaries.
...
| Fuzzy | Term dictionary, edit-distance window | Use `{!fuzzy}` with `prefixLength` to bound cost |
...
- **Term frequency (TF) with saturation.** Mentioning "stranger" 50 times in a synopsis is not 50× better than mentioning it once — repeated mentions yield diminishing returns.
...
The two tuning knobs — `k1` for saturation, `b` for length normalization — are covered in §3.5, where you can actually act on them: the formula, the defaults, and the counter-intuitive directions of both levers.
