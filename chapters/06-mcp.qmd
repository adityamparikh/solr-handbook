...
├── indexing/IndexingService         // index-json / -csv / -xml
...
|---|---|---|
| `solr://collections` | JSON list of collection names. | `@McpResource(uri = "solr://collections")` |
| `solr://{collection}/schema` | JSON schema dump for `{collection}`. | `@McpResource(uri = "solr://{collection}/schema")` |
...
The Solr MCP Server has five completion handlers, all sharing one data path: one bound to the `solr://{collection}/schema` resource template, plus four prompt-argument wrappers (for the `search-collection`, `index-data`, `view-schema`, and `design-schema` prompts) that delegate to it. The resource-template handler:
...
The prompt-argument variant works the same way: `@McpComplete(prompt = "search-collection")` powers the dropdown when the user types into the `collection` arg of the `/search-collection` slash-command — that's what the four wrapper handlers do, each a one-line delegation to the method above.
...
That's the entire user-visible API: 11 tools, 2 resources, 6 prompts, 5 completion handlers. No LTR tool, no signals tool, no `relatedness()` tool — yet. §6.10 covers how you'd add them.
...
The Solr MCP Server's 11 tools, 6 prompts, 2 resources, and 5 completion handlers are a starting set, not a ceiling. If you're building your own primitives (which you should: this is the gap chapter for the MCP book in the series), the design rules vary by primitive type. The first three rules below apply to *tools* — they're the highest-stakes surface because the LLM picks them autonomously. The fourth and fifth cover prompts and completions.
