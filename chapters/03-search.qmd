...
2. **JSON Facet API** (recommended). Nested, composable, supports aggregations like `avg`, `percentile`, `unique`. Introduced in Solr 6.0, per Yonik Seeley's original announcement on yonik.com (April 17, 2015): *"Solr 5 has a completely re-written faceted search and analytics module with a structured JSON API."*
...
HNSW is a **layered proximity graph**. Every vector you index becomes a node. Edges connect each node to its approximate nearest neighbors (up to `M = hnswMaxConnections`, default 16). The trick: not every node lives on every layer. A small fraction of nodes are randomly promoted to higher layers, where they form a sparser graph with longer edges. The top layer has only a handful of nodes; the bottom layer (layer 0) has every node.
