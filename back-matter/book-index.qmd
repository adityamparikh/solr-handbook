...
- caches (`filterCache`, `queryResultCache`, `documentCache`), **§3.1**; autowarming, §3.1; monitoring, §4.14
- `cache=false` local param, **§3.1**
...
- configset, **§4.8**
- copyField, **§2.5**; destinations must be `stored=false` and `useDocValuesAsStored=false`, §2.8.1
- core (Solr), §2.1
- cosine similarity, **§3.6** (illustrative example)
- cuVS GPU codec, §3.6, §7.6 (experimental)
- cursorMark, **§2.9.4**; vs. `start`/`rows` deep paging, §3.1

### D

- debugging queries (`debug=true`, `[explain]`), **§3.10**
- deep paging, §3.1 — see also *cursorMark*
- deletes (soft, live-docs bitset), §1.3
- DenseVectorField, **§3.6**; quantized variants, §3.6; `cagra_hnsw` (GPU), §3.6
- `{!vectorSimilarity}` parser, §3.6
...
- Docker image (Docker Official Image `solr`), §2.7
...
- filter query (`fq`), **§3.1**; caching, §3.1; post-filters and `cost`, §3.1
...
- placement plugins, **§4.11**; `SimplePlacementFactory` is the default, §4.11
- post-filter (`cost>=100`), **§3.1**; Collapse as a post-filter, §3.9
...
- security advisories and CVEs, §4.13, *References*
- `blockUnknown` (set it explicitly), **§4.13**, Appendix A
- `shards.tolerant` / `partialResults`, §3.1
- `security.json`, **§4.13**
