...
- **SOLR-8261 / SOLR-8329** — BM25Similarity becomes default (Solr 6.0; the Lucene-side switch was LUCENE-6789).
...
- **SOLR-14014** — `SOLR_UI_ENABLED` switch for disabling the Admin UI.
- **SOLR-14070** — `CloudSolrClient.Builder` ZooKeeper-host constructor deprecation (fix version 10.1 — *not* deprecated in 10.0.0).
- **SOLR-14656** — Autoscaling framework removed (Solr 9.0).
- **SOLR-15150** — `update.partial.requireInPlace` request parameter (Solr 8.9 / 9.0).
...
- **SOLR-17928** — `efSearchScaleFactor` parameter for the `knn` query parser (Solr 10.0).

Browse: <https://issues.apache.org/jira/projects/SOLR>

### Security advisories and CVEs

- **Apache Solr security news** (the page to check before pinning a version). <https://solr.apache.org/security-news.html>
- **19 May 2026 advisory** — *"JWT Authentication: blockUnknown default allows unauthenticated access contrary to documentation."* Affects Solr 9.0.0–9.10.1 and 10.0.0; no CVE assigned. Mitigation: set `blockUnknown` explicitly (§4.13).
- **CVE-2026-22022** — Unauthorized bypass of certain predefined-permission rules in the `RuleBasedAuthorizationPlugin`. Fixed in 9.10.1.
- **CVE-2026-22444** — Insufficient file-access checking in standalone core-creation requests. Fixed in 9.10.1.
